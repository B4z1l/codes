...
/*Lagrangiana*/
double  lagrangiana(double w2, ps posvel, int sistema, double l){
 double E;
    E =  posvel.v * posvel.v -  potenziale(w2, posvel.x, sistema, l);
    return E;
}
...
/*EULERO*/
ps eulero(double dt, double w2, double beta, ps posvelo, double f0, double wf, int sistema, double alf) { 
     ps posveln;
    
    posveln.v = posvelo.v + oscillatoreforzato(w2, posvelo.x, beta, posvelo.v, f0, wf, posvelo.t, sistema, alf) * dt;
...
/*EULERO-CROMER*/
ps eulero_cromer(double dt, double w2, double beta, ps posvelo, double f0, double wf, int sistema, double alf) { 
   ps posveln;
...
ps verlet_autosufficiente(double dt, double w2, double beta, ps posvelo, double f0, double wf, int sistema, double alf) {
    ps posveln;
    
    posveln.x = posvelo.x + posvelo.v * dt + 0.5 * oscillatoreforzato(w2, posvelo.x, beta, posvelo.v, f0, wf, posvelo.t, sistema, alf) * dt * dt;
    posveln.v = posvelo.v + (oscillatoreforzato(w2, posvelo.x, beta, posvelo.v, f0, wf, posvelo.t, sistema, alf) + oscillatoreforzato(w2, posveln.x, beta, posvelo.v, f0, wf, posvelo.t, sistema, alf)) * 0.5 * dt;
    posveln.t = posvelo.t + dt;
    return posveln;
...
ps rounge_kutta2(double dt, double w2, double beta, ps posvelo, double f0, double wf, int sistema, double alf){
  ...
  agg.x = posvelo.v * dt;
  agg.v = oscillatoreforzato(w2, posvelo.x, beta, posvelo.v, f0, wf, posvelo.t, sistema, alf) * dt;
  

  posveln.x = posvelo.x + ( posvelo.v + 0.5 * agg.v) * dt;
  posveln.v =  posvelo.v + oscillatoreforzato(w2, posvelo.x + 0.5 * agg.x, beta, posvelo.v + 0.5 * agg.v, f0, wf, posvelo.t, sistema, alf) * dt;
  posveln.t = posvelo.t + dt;
            ...
            switch (algoritmo){
              case 0:       /*EULERO*/
                  for (i = 0; i < N; i++) {
                       ...
                       posvel = eulero(dt, w2, beta, posvel, f0, wf,  sistema, alf);
              ...
              case 1:    /*EULERO-CROMER*/ 
                  for (i = 0; i < N; i++) {
                       ...
                       posvel = eulero_cromer(dt, w2, beta, posvel, f0, wf, sistema, alf);
                        E = energia(w2, posvel, sistema, l);
              ...
              case 2:       /*VERLET AUTOSUFFICIENTE*/  
                  for (i = 0; i < N; i++){
                       posvel = verlet_autosufficiente(dt, w2, beta, posvel, f0, wf, sistema, alf);
                        E = energia(w2, posvel, sistema, l);
              ...
              case 3: 
                ...
                switch(c) {
                  case 3:
                               /*rounge-kutta 2*/ 
                    for (i = 0; i < N; i++){
                       posvel = rounge_kutta2(dt, w2, beta, posvel, f0, wf, sistema, alf);
                        E = energia(w2, posvel, sistema, l);
                  ...
                  case 4:     
                              /*rounge-kutta 4*/
                    for (i = 0; i < N; i++){
                       posvel = rounge_kutta4(dt, w2, beta, posvel, f0, wf, sistema, alf);
                        E = energia(w2, posvel, sistema, l);
                        L = lagrangiana(w2, posvel, sistema, l);

                    fprintf(fp, " %lf %lf %lf %lf %lf %lf %lf %lf\n", posvel.t, posvel.x, posvel.v, posvel.v / 2.,-posvel.x / 2., (E-E0)/E0, L, E);
                    }
                    break;
...
void errori(){
 double dt, w2, beta,  wf, tmax, f0, dtmax, t, E, E0, dE,l, alf ;
 int  sistema;
 ps posvel;

FILE *err = fopen("outputplotting/data/err.dat", "w");
...
posvel.v = 1.;
   ...
   for(t = 0; t < tmax; t += dt){
     posvel = verlet_autosufficiente(dt, w2, beta, posvel, f0, wf, sistema, alf); //cambiando il nome della funzione si può valutare l'ordine dell'algoritmo scelto (sono tutte uguali le funzioni degli algoritmi)
    }
...
FILE *fp = fopen("output/plotting/data/fasi.dat", "w");
FILE *poin = fopen("output/plotting/data/poincare.dat", "w");

if(fp == NULL || poin == NULL){
  fprintf(stderr, "Errore: file fasi.dat o poincare.dat non pervenuti.");
  exit(EXIT_FAILURE);
}
double  dt, w2, beta,  f0, wf, T,tmax,  t, alfa, alf = 0.1;
  int  sistema;
  ps posvel;
...
double alf = 0.1;
 ...
 double var = 500.;
   FILE *fp;
   fp  = fopen("output/plotting/data/attrazione.dat", "w");
...
for(j = 0; j < (int)sizeof(forze); j++){ /*ciclo di esecuzione del cambio di forzante*/
 ...
 if(j==0){
    FILE *fx1;
    fx1 = fopen("output/plotting/data/x1.dat", "w");
     f = forze[0];
    for (i = 1; i<=N; i++){
...
posvel.t = 0;
     ...
     FILE*fx2;
     fx2 = fopen("output/plotting/data/x2.dat", "w");
...
posvel.t = 0;
       ...
       FILE*fx3;
     fx3 = fopen("output/plotting/data/x3.dat", "w");
     f = forze[2];
    for (i = 1; i<=N; i++){
...
posvel.t = 0;
     ...
     FILE*fx4;
     fx4 = fopen("output/plotting/data/x4.dat", "w");
     f = forze[3];
    for (i = 1; i<=N; i++){
...
posvel.t = 0;
     ...
     FILE*fx5;
     fx5 = fopen("output/plotting/data/x5.dat", "w");
     f = forze[4];
    for (i = 1; i<=N; i++){
